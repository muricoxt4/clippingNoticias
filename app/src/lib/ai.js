...
function formatPersonaContext(persona, historyItems = []) {
  ...
  if (historyItems.length) {
    blocks.push(
      'Historico recente que nao pode se repetir. ' +
      'Considere como repetido o mesmo fato, movimento, anuncio ou materia, ' +
      'mesmo se o titulo atual estiver reescrito:\n' +
      `- ${historyItems.join('\n- ')}`,
    );
  }
...
- nunca repita itens listados no historico recente
- trate como repetida a mesma noticia mesmo com titulo diferente, se o link, a chamada, o resumo ou o contexto indicarem o mesmo fato
...
export async function filterByPersona(groq, articles, persona, model = DEFAULT_GROQ_MODEL) {
  const articleList = articles
    ...
    .filter(({ article }) => !article.error && article.title && article.link)
