...
function validateEnv() {
  ...
  if (!sites.length) {
    throw new Error(
      ...
      '       Adicione URLs separadas por virgula no .env.',
    );
  }
  ...
  if (!fs.existsSync(personasPath)) {
    throw new Error(
      ...
      '       Copie personas.example.json para personas.json e preencha os perfis.',
    );
  }

  const historyConfig = resolveNewsHistoryConfig(process.env, __dirname);

  return { sites, googleAuth, personasPath, historyConfig };
}
